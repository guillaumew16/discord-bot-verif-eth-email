...
Remarks:
	...
	- Once you receive the email, you have ${config.tokenLifetime} hours to accomplish step 3, as the token expires after that duration.
...
To finish the verification process, please check your Direct Message channel with me (${botName}) and send me the following token within ${config.tokenLifetime} hours: 
${token}
If you did not join the Discord server ${theGuild.name} and tell me your nethz, then someone else provided your nethz. Then you don't need to do anything; the token will expire in ${config.tokenLifetime} hours.
...
client.on('message', async message => {
	...
	if (command === 'ping') {
		...
		if (!args.length) {
			...
			// TODO: save newToken somewhere, along with user.username and user.id
		...
		if (!args.length) {
			...
			const token = args[0];
			const trueToken = "asdfjkl"; // TODO: recover the true token associated with user.username and user.id
			if (token === trueToken) {
